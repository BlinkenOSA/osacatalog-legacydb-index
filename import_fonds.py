...
def main():
    ...
    if args_fonds:
        sql_string = ' AND fonds.ID = ' + args_fonds

    sql = '''
    SELECT * FROM fonds INNER JOIN isad ON fonds.IsadId = isad.Id
...
def make_json(row, lang="en"):
    ...
    j = {"id": hashids.encode(row["ID"] * 1000000), "referenceCode": "HU OSA " + str(row["ID"])}
    ...
    values = select_extent(row["ID"], None, None, lang)
    if values:
        j["extent"] = values
    ...
    values = select_isaar(row["ID"], None, None)
    if values:
        for isaar in values:
...
def make_json_hu(row):
    json_hu = {}
    sql = '''SELECT * FROM fonds INNER JOIN isad2 ON fonds.IsadId = isad2.Id
...
def make_solr_document(row):
    ...
    identifier = hashids.encode(row["ID"] * 1000000)
    ...
    doc = {
        ...
        "archival_reference_number": "HU OSA %s" % (str(row["ID"])),
