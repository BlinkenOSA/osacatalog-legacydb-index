...
from common_archival_unit_functions import select_isaar, select_languages, select_related_units, select_themes, \
                                           select_fonds_name, select_subfonds_name, select_extent, \
                                           make_date_created_display, make_date_created_search
...
def main():
    ...
    if args_fonds:
        sql_string = ' AND series.FondsID = ' + args_fonds

    sql = '''
    SELECT * FROM series INNER JOIN isad ON series.IsadId = isad.Id
    WHERE DatePublic IS NOT NULL
        AND CONCAT_WS("-", series.FondsID, series.SubfondsID, series.ID) NOT IN
            (SELECT reference_code FROM exclude_series) %s
    ORDER BY series.FondsId, series.SubfondsId, series.ID
    ''' % sql_string
    ...
    for row in cursor:
        ...
        solr_interface.add([doc], commit=False)

    solr_interface.commit()
    solr_interface.optimize()

    print 'Import Series - Finished!'


def make_json(row, lang='en'):
    creator = []
    hashids = Hashids(salt="osaarchives", min_length=8)
    j = {"id": hashids.encode(row["FondsID"] * 1000000 + row["SubfondsID"] * 1000 + row["ID"]),
         "referenceCode": "HU OSA " + str(row["FondsID"]) + '-' + str(row["SubfondsID"]) + '-' + str(row["ID"])}
    ...
    values = select_extent(row["FondsID"], row["SubfondsID"], row["ID"], lang)
    if values:
        j["extent"] = values
    ...
    values = select_isaar(row["FondsID"], row["SubfondsID"], row["ID"])
    if values:
        for isaar in values:
...
def make_json_hu(row):
    json_hu = {}
    sql = '''SELECT * FROM series INNER JOIN isad2 ON series.IsadId = isad2.Id
...
def make_solr_document(row):
    ...
    id = hashids.encode(row["FondsID"] * 1000000 + row["SubfondsID"] * 1000 + row["ID"])
    ...
    doc = {
        ...
        "description_level_facet": "Series",
        ...
        "subfonds_name": select_subfonds_name(row["FondsID"], row["SubfondsID"]),
